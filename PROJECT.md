...
> Verzia 2.13.0 — Post-Sprint 8 údržba: automatická QR detekcia z fotky štítku pri uploade; webcam na desktope; responzívny layout a vylepšená inventárna tabuľka (fullscreen, lightbox, náhľad fotiek vedľa tabuľky).
...
| 41 | **QR detekcia z LABEL fotky pri uploade** — server dekóduje QR z obrázka štítku a pokúsi sa priradiť položke | V teréne je QR často na tom istom snímku ako štítok. `POST /api/items/:id/photos` (len `photo_type = LABEL`) po uložení fotky spustí `detectQrFromImage` (sharp + `@zxing/library`, multi-scale 768/1024/1280 px, timeout 2 s, nikdy nepreruší upload). `tryAssignDetectedQr` atomicky priradí tag ak položka nemá QR a kód existuje vo `QRTag`. Odpoveď obsahuje `qrDetection` so stavmi ASSIGNED / NO_QR_DETECTED / NOT_FOUND / ALREADY_ASSIGNED / ITEM_HAS_QR. FE banner + odkaz „Naskenovať ručne". Gemini prompt (`base.md`) ignoruje QR/čiarové kódy v OCR texte. |
...
**Upload LABEL (2.13):** odpoveď `POST /api/items/:id/photos` okrem `signed_url` obsahuje `qrDetection` — výsledok server-side dekódovania QR z nahraného obrázka (pozri §4.11).
...
- **LABEL upload + QR:** synchronne pred odpoveďou (max ~2 s) — ak je na fotke QR nálepky a položka ešte nemá `qr_code`, systém ho priradí; inak banner s dôvodom a odkazom na `/scan?assignTo=…`
...
**Fullscreen a náhľad fotiek (2.13):** tlačidlo „Celá obrazovka“ — natívny Fullscreen API (desktop) + CSS overlay fallback (iOS); po `confirm()` pri mazaní sa fullscreen obnoví; modál stĺpcov a filter menu cez portál s vyšším z-indexom. Klik na počet fotiek → `PhotoLightbox` (čerstvý signed URL) alebo „Otvoriť vedľa“ (`openPhotoBesideById`) — úzke popup okno vpravo, tabuľka ostáva vľavo.

### 4.11 QR detekcia z fotky štítku

**Backend:** `services/qrDetection.ts`, `services/qrAssignment.ts`; volané z `routes/photos.ts` po `POST` uploadu LABEL fotky.

| Stav `qrDetection` | Význam |
|---|---|
| `ASSIGNED` | QR rozpoznaný a priradený položke |
| `NO_QR_DETECTED` | Na fotke sa nenašiel QR (alebo timeout) |
| `NOT_FOUND` | Dekódovaný text nie je v tabuľke `QRTag` |
| `ALREADY_ASSIGNED` | QR už patrí inej položke |
| `ITEM_HAS_QR` | Položka už má QR — ticho, bez banneru |

**Frontend:** `PhotoUpload` — po uploade `QrDetectionBanner`; pri chybe link na manuálny scan. Platí len pre LABEL; OVERVIEW QR nečíta.

**Upload UI (2.13):** štyri sloty (štítok/položka × kamera/galéria); mobilná kamera cez `capture="environment"`; desktop kamera cez `WebcamCaptureModal` (`getUserMedia`); kompresia nad 2 MB pred uploadom.
...
→ skenuj QR krabice (alebo odfotím štítok s QR — systém ho priradí pri LABEL uploade)
→ otvor krabicu → každej zložke odfot štítok (LABEL) → ak je na snímke QR nálepky, priradí sa automaticky
→ OCR → metadata batch doma
```
...
### UC-7: Priradiť QR bez samostatného skenu
```
V detaile položky (bez QR) → Odfotiť štítok (LABEL)
→ server dekóduje QR z fotky → banner „QR QR-000042 rozpoznaný a priradený"
→ ak QR na fotke nie je / neexistuje v DB → varovanie + „Naskenovať ručne" → /scan?assignTo=…
```
...
| QR scan | @zxing/browser (kamera ScanPage) + manuálny input + server-side detekcia z LABEL fotky (`sharp` + `@zxing/library`, multi-scale) | ✓ live (Sprint 2 + 2.13) |
| Foto upload UI | 4 sloty LABEL/OVERVIEW; mobil kamera; desktop `WebcamCaptureModal`; kompresia >2 MB | ✓ live (2.13) |
...
| Inventárna tabuľka | Strom + výber stĺpcov (localStorage), Excel-style filtre/zoradenie (URL `cf`, `sort`), fullscreen (natívny + iOS fallback), lightbox + náhľad vedľa tabuľky | ✓ live (Sprint 7 + 2.11–2.13) |
...
│   ├── src/services/qrDetection.ts, qrAssignment.ts
...
### Údržba po Sprint 8 (máj 2026) ✓ pokračuje (2.11–2.13)
...
**Terénny UX a QR z fotky (2.13)**
- ✓ Backend: `qrDetection.ts` (sharp resize, multi-scale, 2 s timeout), `qrAssignment.ts`, integrácia v `POST` upload LABEL
- ✓ FE: `PhotoUpload` — `QrDetectionBanner`, 4 upload sloty, invalidácia `["item", itemId]` po priradení QR
- ✓ `WebcamCaptureModal` — desktop kamera namiesto file inputu bez `capture`
- ✓ `utils/device.ts` — `isMobileDevice()` pre výber kamera vs webcam
- ✓ Responzívny layout: `styles.css`, formulár vytvorenia položky, `ItemDetailPage`, `ItemsPage`
- ✓ `ItemsDataTable` — vylepšený fullscreen (obnova po confirm, z-index modalov), `PhotoLightbox` s čerstvým signed URL
- ✓ `openPhotoBeside.ts` — náhľad fotky v popup okne vedľa tabuľky (`openPhotoBesideById`)
- ✓ `backend/prompts/base.md` — QR/čiarové kódy ignorované v Gemini OCR
...
| TD-6 | ~~Chýba QR scan pri "Pridať podradeú položku"~~ | ✓ hotové — `AddChildMethodChooser` → `/scan?parentId=…` (2.11) |
...
*Posledná aktualizácia: v2.13.0 — máj 2026. Od v2.12.0: QR detekcia a auto-priradenie z LABEL fotky (`qrDetection` + `qrAssignment`); `WebcamCaptureModal` na desktope; responzívny layout; inventárna tabuľka — fullscreen (z-index, obnova po confirm), lightbox s čerstvým URL, náhľad fotiek vedľa tabuľky. TD-6 uzavreté. Obnova zmazaných (koš / restore) zatiaľ nie — TD-21.*
*Ďalší krok: field work v sklade (Sprint 6), overiť `R2_PUBLIC_URL` (TD-23), potom TD-21 / TD-5 / TD-7 / TD-12 podľa potreby.*